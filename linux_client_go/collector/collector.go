...
var avKeywords = []string{
	// 国产
	"360safe", "360-safeguard", "huorong", "qax", "tianqing",
	"sangfor", "edr-sangfor", "rising",
	// 国外
...
func getInstalledPackages() map[string]pkgInfo {
	...
	// dpkg 同时获取版本和安装日期
	dpkgOut := runCmd("dpkg-query -W -f='${Package}|${Version}|${Installed-Size}\\n' 2>/dev/null", 3*time.Second)
	if dpkgOut != "" {
...
func getDpkgInstallDate(pkgName string) string {
	// 方法1: 从 dpkg info 的 mtimes 获取
	out := runCmd("ls -lt --time-style=+%Y-%m-%d /var/lib/dpkg/info/"+pkgName+".list 2>/dev/null | head -1", 1*time.Second)
	if out != "" {
	...
	// 方法2: 从 /var/log/dpkg.log* 获取
	out2 := runCmd("zgrep -h 'install "+pkgName+" ' /var/log/dpkg.log* 2>/dev/null | tail -1", 2*time.Second)
	if out2 != "" {
		re := regexp.MatchString
		_ = re
		fields := strings.Fields(out2)
		if len(fields) >= 3 {
			dateStr := fields[0]
